...
async function runAgentC({ scriptText, annotatedScript = '', costumeCard = '', config, options = {}, onEvent, directorNotes = '', annotation = null }) {
  // 强制写文件确认本函数被调用
  const fs = require('fs');
  const logPath = 'D:\\AI\\project\\test\\test_call.log';
  try { fs.appendFileSync(logPath, `[${new Date().toISOString()}] runAgentC 被调用，有config=${!!config}，有apiKey=${!!config?.apiKey}\n`); } catch(e) { console.log('[test_call.log 写入失败]', e.message); }
  ...
  for (const scene of scenes) {
    ...
    let prevSegmentEnd = ''; // 上一片段末帧画面，用于下一片段AI增强时的上下文
    for (const segment of segments) {
      emit({ type: 'model_start', stage: 'AGENT_C', sceneId: scene.id, message: `生成片段${segment.id}硬锁版：跳过C模型自由写作，按parser账本直接渲染` });
      const skeleton = createSegmentSkeleton(scene, segment, cleanB, options.visualStyle || 'plain', { annotation, directorNotes });
      ...
      // Batch Enrich: AI增强（每个片段1次调用，非逐镜头）
      console.log(`[Pipeline] 准备调用enrichSegmentShots，segment=${segment.id}，有config=${!!config}，有apiKey=${!!config?.apiKey}`);
      const enriched = await enrichSegmentShots({ scene, segment, parsed, costumeCard: cleanB, config, prevSegmentEnd, directorNotes, annotation });
      console.log(`[Pipeline] enrichSegmentShots 返回，enriched !== parsed = ${enriched !== parsed}`);
      if (enriched !== parsed) {
        debugWrite(options, `${scene.id}_${segment.id}_enriched.json`, enriched);
      }

      // 提取末帧画面，供下一片段使用
      const lastShot = (enriched.shots || []).slice(-1)[0];
      const lastDl = (lastShot?.dialogueLines || [])[0];
      const lastSize = lastShot?.shotSize || '中景';
      const lastPosition = /侧面/.test(lastSize) ? '侧面' : /正面/.test(lastSize) ? '正面' : '中景';
      prevSegmentEnd = lastDl
        ? `${String(lastDl.speaker || '角色')}：${lastSize}${lastPosition ? '·' + lastPosition : ''}`
        : `${String(lastShot?.task || segment.title || segment.id)}：${lastSize}`;
...
async function runFull({ scriptText, directorNotes = '', mode = 'ai', config, options = {}, onEvent }) {
  ...
  const c = await runAgentC({ scriptText, annotatedScript: a.output, costumeCard: b.output, config, options, onEvent, directorNotes });

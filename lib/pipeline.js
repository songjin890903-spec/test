  ...
  for (const scene of scenes) {
    ...
    for (let segIdx = 0; segIdx < segInfos.length; segIdx++) {
      ...
      if (enriched && enriched.shots && enriched.shots.length) {
        const hasAnyDialogue = enriched.shots.some(s => (s.dialogueLines || []).length > 0);
        const MAX_SHOTS = hasAnyDialogue ? 7 : 5;
        if (enriched.shots.length > MAX_SHOTS) enriched.shots = enriched.shots.slice(0, MAX_SHOTS);

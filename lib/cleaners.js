...
function cleanAgentBForC(output) {
  ...
  let manualSupplement = extractBlockByTitle(s, '【用户补充服化道/视觉要求】');
  if (!manualSupplement) {
    const manualTitle = '【用户补充服化道/视觉要求】';
    const manualIdx = s.indexOf(manualTitle);
    if (manualIdx >= 0) manualSupplement = s.slice(manualIdx).trim();
  }
  const parts = [];
  if (picture) {
  ...
  if (manualSupplement) parts.push(manualSupplement);
